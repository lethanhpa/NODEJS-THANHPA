...
router.get("/5", function (req, res, next) {
  const birthday = req.query.q;
  try {
    const query = {
      $expr: {
        $eq: [{ $year: "$birthday" }, birthday],
      },
    };
...
router.get("/8a", function (req, res, next) {
  try {
    ...
    const compareStatus = { $eq: ["$status", /*status*/ "WAITING"] };
...
router.get("/15", function (req, res, next) {  
  try {
    const supplierNames = req.query.name;
...
// ------------------------------------------------------------------------------------------------
router.get("/26", async (req, res, next) => {
  try {
    const aggregate = [
      {
        $lookup: {
